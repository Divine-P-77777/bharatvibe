...
>(({ className, variant = "default", ...props }, ref) => {
  const toastClass =
    variant === "destructive"
      ? "bg-destructive text-destructive-foreground border border-orange-500"
      : "bg-background text-foreground border border-orange-500"

  return (
    <ToastPrimitives.Root
      ref={ref}
      className={`
        group pointer-events-auto relative w-full items-center justify-between space-x-4 overflow-hidden
        rounded-xl border p-6 pr-8 shadow-md transition-all
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ...
    className={`
      ...
      group-[.destructive]:border-muted/40 group-[.destructive]:hover:border-orange-500
      group-[.destructive]:hover:bg-destructive group-[.destructive]:hover:text-destructive-foreground
      group-[.destructive]:focus:ring-orange-500
      ${className}
    `}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ...
    className={`
      ...
      group-[.destructive]:text-orange-300 group-[.destructive]:hover:text-orange-100 group-[.destructive]:focus:ring-orange-500
      ${className}
    `}
    toast-close=""
    {...props}
