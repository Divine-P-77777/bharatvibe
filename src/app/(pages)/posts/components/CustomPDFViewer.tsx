...
const CarouselCore: React.FC<{
    ...
    url, downloadName, onExpand, showExpand, showDownload, overlay = false, onCloseOverlay
        ...
        const setupWebViewer = useCallback(() => {
            ...
            // Clean up previous instance if there happens to be one
            ...
            // Create a dedicated inner element for the WebViewer instance
            const innerViewer = document.createElement('div');
            innerViewer.style.width = '100%';
            innerViewer.style.height = '100%';
            viewer.current.appendChild(innerViewer);
            ...
            WebViewer(
                ...
                innerViewer // Mount to the dynamically created element
            ).then((instance: any) => {
        ...
        return (
            ...
            >
                {showExpand && (
                    <button onClick={onExpand} className="absolute top-4 right-4 z-20 p-2 bg-black/60 hover:bg-black/80 rounded-full shadow-lg backdrop-blur-sm transition-colors" aria-label="View Full">
                        {ICON_EXPAND}
                    </button>
                )}
                {showDownload && (
                    <button onClick={handleDownload}
                        className="absolute top-4 right-4 z-20 p-2 bg-black/60 hover:bg-black/80 rounded-full shadow-lg backdrop-blur-sm transition-colors"
                ...
                {overlay && (
                    <button onClick={onCloseOverlay}
                        className="absolute top-4 left-4 z-20 p-2 bg-black/60 hover:bg-black/80 rounded-full shadow-lg backdrop-blur-sm transition-colors"
